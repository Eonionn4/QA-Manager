...
function filterRelSearchList() {
  const keyword = document.getElementById('relSearchInput').value.toLowerCase();
...
function loadUnifiedLotList() {
  ...
  Promise.all([
    ...
    const all = [
      ...flattenLotNPI(npiSnap.docs),
      ...flattenLotSchedule(scheduleSnap.docs),
      ...flattenLotFA(faSnap.docs)
    ];
    renderUnifiedLotTable(all);
  });
}
