...
function renderHistoryList() {
  ...
  sortedEntries.forEach((entry) => {
    const actualIndex = historyEntries.indexOf(entry);
    ...
    block.innerHTML = `
      ...
      <div style="margin-bottom: 8px;">
        <button class="lot-btn add" onclick="toggleLotInput(${actualIndex})">📦 Lot Info</button>
        <button class="lot-btn edit" onclick="editHistory(${actualIndex})">✏️ Edit</button>
        <button class="lot-btn delete" onclick="deleteHistory(${actualIndex})">❌ Delete</button>
      </div>
      <div id="lot-input-${actualIndex}" class="lot-input-area" style="display:none;">
        <input id="wafer-${actualIndex}" placeholder="Wafer Info" />
        <input id="device-${actualIndex}" placeholder="Device No" />
        <input id="run-${actualIndex}" placeholder="Run No" />
        <input id="lot-${actualIndex}" placeholder="Lot No" />
        <input type="number" id="inQty-${actualIndex}" placeholder="IN Qty" />
        <input type="number" id="outQty-${actualIndex}" placeholder="OUT Qty" />
        <div id="rejArea-${actualIndex}" class="rej-entry-wrapper">
          <div class="rej-row">
            <input type="number" placeholder="Rej Qty" class="rejQty-${actualIndex}" />
            <input type="text" placeholder="Rej Mode" class="rejMode-${actualIndex}" />
            <input type="text" placeholder="Rej Process" class="rejProcess-${actualIndex}" />
            <button onclick="addRejEntry(${actualIndex})" class="lot-btn add">➕</button>
          </div>
          <div id="rejList-${actualIndex}" class="rej-list"></div>
        </div>
        <div class="lot-input-actions">
          <button id="addLotBtn-${actualIndex}" onclick="addLotInfo(${actualIndex})" class="lot-btn add">➕ Add Lot</button>
          <button id="reviseLotBtn-${actualIndex}" onclick="reviseLotInfo(${actualIndex})" class="lot-btn revise" style="display:none;">🔁 Revise Lot</button>
        </div>
      </div>
      <div id="lot-list-${actualIndex}" style="margin-top:10px;"></div>
    `;
    container.appendChild(block);
    renderLotList(actualIndex); // ✅ key fix here
  });
}
...
function renderLotList(index) {
    ...
    <div class="lot-info-entry-grid" data-device="${lot.device}" data-lot="${lot.lot}">
      ...
      <div><b>Rej:</b> ${
        Array.isArray(lot.rejEntries)
          ? lot.rejEntries.map(e => `${e.qty}×${e.mode} (${e.process})`).join(", ")
          : "-"
      }</div>
      <div class="lot-btns">
  ...
  // ✅ Highlight if redirected from Sheet15
  setTimeout(() => {
...
function addLotInfo(index) {
  ...
  // 🔁 Clear Rej entries after adding
  rejEntryMap[index] = [];
  renderLotList(index);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("addHistoryBtn").addEventListener("click", () => {
    const entry = {
      ...
      lotInfo: [] // ✅ new independent lotInfo per entry
    };

    historyEntries.push(JSON.parse(JSON.stringify(entry))); // ✅ Deep copy to prevent reference bugs
    renderHistoryList();
    clearHistoryInputs();
  });
  ...
  document.getElementById("saveNPI").addEventListener("click", async () => {
    ...
    try {
      ...
      // ✅ Also update devicesy_cache
      data.history.forEach(entry => {
        (entry.lotInfo || []).forEach(lot => {
          if (!lot.lot) return; // 🚫 skip if lot.lot is empty
...
function loadDeviceSyTable() {
  ...
  Promise.all([
    ...
    renderDeviceSyResults(fullDeviceSyList); // ✅ initial render
  });

  // ✅ Add search filter ONCE
...
const rejEntryMap = {}; // index → temp array of rej entries
...
function renderRejList(index) {
  const entries = rejEntryMap[index] || [];  // ✅ define entries first
  ...
  list.innerHTML = entries.map((e, i) => `
    <div style="background:#eef6ff; padding:4px 8px; border-radius:6px; display:flex; align-items:center;">
      ${e.qty}×${e.mode} (${e.process})
      <button class="lot-btn delete" style="margin-left:6px; padding: 2px 6px;" onclick="removeRejEntry(${index}, ${i})">❌</button>
    </div>
  `).join("");
}
