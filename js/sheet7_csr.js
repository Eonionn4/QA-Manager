...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Updated loadTable with highlight support
  async function loadTable(highlightSpecs = []) {
    tableBody.innerHTML = "";
    const snapshot = await getDocs(collection(db, "csr_masterlist"));
    snapshot.forEach(docSnap => {
      const d = docSnap.data();
      ...
      if (highlightSpecs.includes(d.SPECNUM)) {
  ...
  // ✅ Import Excel logic with highlight tracking
  fileInput.onchange = async (e) => {
    ...
    reader.onload = async function (e) {
      ...
      loadTable(highlightSpecs); // ✅ Highlight new/revised rows
    };
