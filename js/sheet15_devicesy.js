...
function isCustomerAllowed(customerName = "") {
  ...
  return allowedCustomers.includes("all") || allowedCustomers.some(keyword => lower.includes(keyword));
}
...
function flushGlobalBuffer() {
  ...
  // Dynamic buttons for each source
  ...
  globalRowBuffer.forEach(data => {
    ...
    // ✅ Skip this row if it doesn't match the current source filter
    ...
    renderRow(data, data.docId || null, editable); // ✅ Editable only for non-system rows
  });

  // ✅ Scroll and highlight based on lot/device passed from another sheet
  setTimeout(() => {
    ...
    rows.forEach(row => {
      const lotCell = row.cells[11];       // Lot No column
      const deviceCell = row.cells[10];    // Device No column
      const sourceCell = row.cells[14];    // Source column
  ...
  // ✅ Re-render Calendar based on filter
  renderFullCalendar(currentSourceFilter);

  // ✅ Re-attach click listeners for all filter buttons
  document.querySelectorAll(".filter-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      currentSourceFilter = btn.dataset.source; // "" = All
      flushGlobalBuffer();
...
function loadAllDeviceData() {
  ...
  db.collection("device_list").orderBy("createdAt", "desc").get().then(snapshot => {
    snapshot.forEach(doc => {
      const data = doc.data();
      addRow(data, doc.id); // ✅ pass doc.id here too
    });
    loadNPIData();
  });
}
...
document.getElementById("searchInput").addEventListener("input", function () {
  ...
  rows.forEach(row => {
    if (row.classList.contains("lot-header")) {
      ...
      // Show the lot header if any of its rows are visible
      if (match && currentLot) {
...
document.getElementById("searchInput").addEventListener("input", function () {
  ...
  rows.forEach(row => {
    if (row.classList.contains("lot-header")) {
      ...
      // Show lot header if its row is visible
      if (match && currentLot) {
        currentLot.style.display = "";
      }
    }
...
function renderFullCalendar(sourceFilter = "") {
  ...
  globalRowBuffer.forEach(row => {
    ...
    const lot = row.lotNo || "-";
    const item = row.item || "-";
    const color = getColorForSource(sourceKey);
    const title = `${source} - ${lot} - ${item}`;
    ...
    const extended = {
      ...
      textColor: "#fff",
      borderColor: "#111",
      extendedProps: { lotNo: lot, item: item }
    };
    ...
    if (inDate && outDate) {
      events.push({
        ...extended,
        start: inDate,
        end: new Date(new Date(outDate).getTime() + 86400000).toISOString().split("T")[0]
      });
    } else if (inDate) {
      events.push({ ...extended, title: `IN: ${title}`, start: inDate });
    } else if (outDate) {
      events.push({ ...extended, title: `OUT: ${title}`, start: outDate });
    }
  });
