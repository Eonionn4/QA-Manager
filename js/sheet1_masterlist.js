...
async function loadFromFirestore(searchKeyword = "") {
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      let inheritedStart = new Date(item.startDate || new Date());
      inheritedStart.setHours(0, 0, 0, 0);

      testListByArea.forEach(([areaId, testList]) => {
        let currentPlanDate = new Date(inheritedStart);

        testList.forEach(test => {
          const duration = parseInt(test.planDuration || test.duration || 0);
          const planInDate = new Date(currentPlanDate);
          const planOutDate = new Date(planInDate);
          planOutDate.setDate(planOutDate.getDate() + duration);

          if (!test.actualIn && today > planInDate) {
            violationMessages.push(`[${areaId}] Input violation - ${test.criteria}`);
          }

          if (!test.actualOut && today > planOutDate) {
            violationMessages.push(`[${areaId}] Test Out violation - ${test.criteria}`);
          }

          currentPlanDate = new Date(planOutDate);
        });

        inheritedStart = new Date(currentPlanDate);
      });
